 ...
 * @author Comp
 */
public class JavaElementCheckerVisitor implements ElementCheckerVisitor
{
  private final ArrayList<ClassElement> classes;
  private final ArrayList<InheritanceRelationship> inheritances;
  private final ArrayList<AggregationRelationship> aggregations;
  
  public JavaElementCheckerVisitor()
  {
  ...
  public String visit(InheritanceRelationship r)
  {
    ...
    if(error.isEmpty())
    {
      Iterator<InheritanceRelationship> inherIt = inheritances.iterator();
      while(inherIt.hasNext())
      {
        InheritanceRelationship inher = inherIt.next();
        if(inher.getSourceClass() == r.getSourceClass())
        {
          error += "Class " + r.getSourceClass().getName();
          error += " inherits multiple classes, not allowed in java! ";
          break;
        }
      }
      inheritances.add(r);
